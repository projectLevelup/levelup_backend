...
public class ProductmakedataService {
	...
	@Transactional
	public void generateReviews(int totalRecords) {
		List<UserEntity> users = userRepository.findAll();
		List<ProductEntity> products = productRepository.findAll();

		if (users.isEmpty() || products.isEmpty()) {
			throw new IllegalStateException("유저 또는 제품 데이터가 없습니다. 먼저 데이터를 생성하세요.");
		}

		IntStream.range(0, totalRecords).forEach(i -> {
			UserEntity randomUser = users.get(random.nextInt(users.size())); // 랜덤 유저 선택
			ProductEntity randomProduct = products.get(random.nextInt(products.size())); // 랜덤 제품 선택

			// 랜덤 주문 생성
			OrderEntity order = OrderEntity.builder()
				.user(randomUser)
				.product(randomProduct)
				.status(random.nextBoolean() ? OrderStatus.COMPLETED : OrderStatus.CANCELED) // 랜덤 상태
				.totalPrice(randomProduct.getPrice()) // 제품 가격 기준
				.orderName("주문-" + (i + 1))
				.build();
			...
			ReviewEntity review = ReviewEntity.builder()
				.user(randomUser)
				.product(randomProduct)
				.order(order) // 위에서 생성한 주문과 연결
				.contents("리뷰 내용 " + (i + 1))
				.starScore(random.nextInt(5) + 1) // 1~5점 랜덤 점수
				.build();

			reviewRepository.save(review); // 리뷰 저장
		});

		System.out.println(totalRecords + "개의 주문 및 리뷰 데이터가 삽입되었습니다.");
	}
